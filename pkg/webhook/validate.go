...
func runGrypeOnSingleImage(sbomFile string, imageName string) bool {
	timeName := formatTime()
	...
	log.Print("Post grype")
	// Crash if there are any errors
	util.NonfatalErrorCheck(err)

	log.Print("Outfile name:" + outFile)
	// Write output to file
	util.WriteFile(outFile, string(out))
...
func checkPodImages(pod *corev1.Pod) (dashboard.DashboardUpdate, error) {
	...
	for range containers {
		...
		grypeRes := runGrypeOnSingleImage(sbomName, containers[counter].Image)
		log.Print("ran grype")
		if grypeRes == true {
			failure = true
		}
